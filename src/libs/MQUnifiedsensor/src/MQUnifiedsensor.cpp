...
void MQUnifiedsensor::serialDebug(bool onSetup)
{
    if (onSetup)
    {
        ...
        Serial.println("Note: remember that all the parameters below can be modified during the program execution with "
                       "the methods:");
...
float MQUnifiedsensor::validateEcuation(float ratioInput)
{
    ...
    else
    {
        ...
        double ppm_log = (log10(ratioInput) - _b) / _a; // Get ppm value in linear scale according to the the ratio
                                                        // value
...
float MQUnifiedsensor::readSensor()
{
    ...
    if (_regressionMethod == 1)
        _PPM = _a *
               pow(_ratio, _b); // <- Source excel analisis
